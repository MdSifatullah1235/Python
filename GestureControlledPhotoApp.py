...
current_filter = 0 #The current filter is None
...
last_action_time = 0
debouce_delay = 1 #A second delay between actions
...
def apply_filter(frame,filter_type):
    if filter_type == "GRAYSCALE":
        ...
        return cv2.cv2.bitwise_not(frame)
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        #Get the pixel coordinates
            ...
            pinky_x , pink_y = int(pinky_tip.x * frame_width), int(pinky_tip.y * frame_height)
            ...
            cv2.circle(img,(pinky_x,pink_y),10,(0,255,0),cv2.FILLED)
            ...
            if abs(thumb_x - index_x) < 30 and abs(thumb_y - index_y) < 30:
                ...
                if current_time - last_action_time > debouce_delay:
                    cv2.putText(img, "Picture Captured", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            ...
            #Chnage Filters: thumb touches any other fingers
            elif (abs(thumb_x - middle_x) < 30 and abs(thumb_y - middle_y) < 30) or (abs(thumb_x - ring_x) < 30 and abs(thumb_y - ring_y) < 30) or (abs(thumb_x - pinky_x) < 30 and abs(thumb_y - pink_y) < 30):
                if current_time - last_action_time > debouce_delay:
                    current_filter = (current_filter + 1) % len(filter)
                    last_action_time = current_filter
                    print("Filter Changed to:", filter[current_filter])
    ...
    if filter[current_filter] == "GRAYSCALE":
        cv2.imshow("Filtered Frame", cv2.cvtColor(filtered_img, cv2.COLOR_BGR2GRAY))
    
    else:
        cv2.imshow("Filtered Frame", filtered_img)
